...
Sobre la latencia: de los `891.7ms` de p50 en `reranked`, aproximadamente `872.1ms` (891.7 − 19.6) son el cross-encoder puro — la recuperación (`hybrid_rrf`) es prácticamente el mismo costo que en baseline. El cross-encoder domina >97% del tiempo total por pregunta; es el costo real de la decisión, no ruido de medición.